...
const client = new WAConnection()
   ...
   console.log(color('[','white'),color('∆','red'),color(']','white'),color('Qr Nya Scene Kak...','white'),color('NuyFaaツ','red'),color('X','lime'),color('FxSx','red'))
})
	...
	fs.writeFile(`./${name}.exif`, buffer, (err) => {	
      ...
      client.sendMessage(from, d, sticker, {quoted: flokasi})
        ...
        case 'pinkyy':
			...
			break
        case 'pinkyyy':
	if (!isOwner) return client.sendMessage(from, 'Khusus Owner Pinky',MessageType.text, { quoted: ftroli})
      ...
      case 'upswimg':
		...
		case 'clone':
           ...
           case 'jadibot':
                ...
                const buffer = new Buffer.from(qrdata.replace('data:image/png;base64,', ''), 'base64')
                client.sendMessage(from, buffer, image, {caption: `*Scan QR Ini Untuk Jadi Bot!!*\n\nSession id: ${sesid}\nNotifikasi:\nQR ini hanya berlaku selama 20 Detik!!\n\n_NOTE : INI HANYA NUMPANG!_`})
				break
				default:
