...
const { wait, getBuffer, h2k, generateMessageID, getGroupAdmins, getRandom, banner, start, info, success, sleep, close } = require('./lib/functions')
...
/********** FUNCTION ***************/
        ...
        const addRegisteredUser = (userid, sender, time, serials) => {
            const obj = { id: userid, sender, time: time, serial: serials }
...
nuy.on('credentials-updated', () => {
   ...
   fs.writeFileSync('./nuyfaa.json', JSON.stringify(authInfo, null, '\t'))
})
fs.existsSync('./nuyfaa.json') && nuy.loadAuthInfo('./nuyfaa.json')
nuy.connect();
...
nuy.on('group-participants-update', async (anu) => {
	if (!welkom.includes(anu.jid)) return
		try {
			...
			if (anu.action == 'add') {
				...
				nuy.sendMessage(mdata.id, buff, MessageType.image, {quoted: {key : {participant : '0@s.whatsapp.net'}, message: {orderMessage: {itemCount : 1, status: 1, surface : 1, message: `Welcome @${num.split('@')[0]}\n${tampilTanggal}`, orderTitle: `Welcome @${num.split('@')[0]}\n${tampilTanggal}`, thumbnail: fs.readFileSync('pinky.jpg'), sellerJid: '0@s.whatsapp.net'} } }, contextInfo: {"mentionedJid": [num]}, caption: teks})
				...
				let buff = await getBuffer(ppimg)
				nuy.sendMessage(mdata.id, buff, MessageType.image, {quoted: {key : {participant : '0@s.whatsapp.net'}, message: {orderMessage: {itemCount : 1, status: 1, surface : 1, message: `Keluar @${num.split('@')[0]}\n${tampilTanggal}`, orderTitle: `Keluar @${num.split('@')[0]}\n${tampilTanggal}`, thumbnail: fs.readFileSync('pinky.jpg'), sellerJid: '0@s.whatsapp.net'} } }, contextInfo: {"mentionedJid": [num]}, caption: teks})
			}
		} catch (e) {
		console.log(e)
	}
	...
	nuy.on('message-update', async (mek) => {
		try {
	   const from = mek.key.remoteJid
		...
		const sender = mek.key.fromMe ? nuy.user.jid : mek.key.remoteJid.endsWith('@g.us') ? mek.participant : mek.key.remoteJid
		...
		if (messageStubType == 'REVOKE') {
			...
			const conts = mek.key.fromMe ? nuy.user.jid : nuy.contacts[sender] || { notify: jid.replace(/@.+/, '') }
			const pushname = mek.key.fromMe ? nuy.user.name : conts.notify || conts.vname || conts.name || '-'
			const opt4tag = {
...
❏ Pesan : *${body ? body : '-'}*
╰──────────`
				nuy.sendMessage(from, strConversation, MessageType.text, { quoted: { key : {participant : '0@s.whatsapp.net'}, message: {orderMessage: {itemCount : 1, status: 1, surface : 1, message: fakereply, orderTitle: fakereply1, thumbnail: fs.readFileSync('pinky.jpg'), sellerJid: '0@s.whatsapp.net'} } }, contextInfo: { forwardingScore: 508, isForwarded: true}})
...
╰──────────`
				...
				nuy.sendMessage(from, strConversation, MessageType.text, { quoted: {key : {participant : '0@s.whatsapp.net'}, message: {orderMessage: {itemCount : 1, status: 1, surface : 1, message: fakereply, orderTitle: fakereply1, thumbnail: fs.readFileSync('pinky.jpg'), sellerJid: '0@s.whatsapp.net'} } }, contextInfo: { forwardingScore: 508, isForwarded: true}})
				nuy.sendMessage(from, buff, MessageType.sticker, { quoted: {key : {participant : '0@s.whatsapp.net'}, message: {orderMessage: {itemCount : 1, status: 1, surface : 1, message: fakereply, orderTitle: fakereply1, thumbnail: fs.readFileSync('pinky.jpg'), sellerJid: '0@s.whatsapp.net'} } }, contextInfo: { forwardingScore: 508, isForwarded: true}})
...
╰──────────`
	...
	fakeimagecuy = fs.readFileSync(`./pinky.jpg`)
	...
	nuy.on('message-new', async (mek) => {
		...
		if (urutan_pesan === 5000) {
			...
			const fgclink = {key: { fromMe: false,remoteJid: "0@s.whatsapp.net", participant: '0@s.whatsapp.net'}, message: {groupInviteMessage: {groupJid: "6283815956151-1616169743@g.us", inviteCode: "mancanihbos", groupName: fakereply, caption: fakereply1, jpegThumbnail: fs.readFileSync('pinky.jpg') } } }
			const fdocs = {key : {participant : '0@s.whatsapp.net'}, message: {documentMessage: {title: fakereply, jpegThumbnail: fs.readFileSync('pinky.jpg') } } }
			const flokasi = {key : {participant : '0@s.whatsapp.net'}, message: {locationMessage: { name: fakereply, jpegThumbnail: fs.readFileSync('pinky.jpg') } } }
			const fvideo = {key: { fromMe: false, participant: '0@s.whatsapp.net', ...(from ? { remoteJid: "6283815956151-1613049930@g.us" } : {}) }, message: { videoMessage: { title: fakereply, seconds: 999, caption: fakereply1, jpegThumbnail: fs.readFileSync('pinky.jpg') } } }
			const fgif = {key: { fromMe: false, participant: '0@s.whatsapp.net', ...(from ? { remoteJid: "6283815956151-1613049930@g.us" } : {}) }, message: { videoMessage: { title: fakereply, seconds: 999, gifPlayback: true, caption: fakereply1, jpegThumbnail: fs.readFileSync('pinky.jpg')} } }
			const ftroli = {key : {participant : `0@s.whatsapp.net`}, message: {orderMessage: {itemCount : 1, status: 1, surface : 1, message: fakereply, orderTitle: fakereply1, thumbnail: fs.readFileSync('pinky.jpg'), sellerJid: `0@s.whatsapp.net`} } }
			const isUrl = (url) => {
	...
	fs.writeFile(`./${name}.exif`, buffer, (err) => {	
          	...
          	let found = false
                    for (let lmt of _limit) {
                        if (lmt.id === sender) {
                            ...
                            if (limitCounts <= 0) return nuy.sendMessage(from,`Limit request anda sudah habis\n\nNote : limit bisa di dapatkan dengan cara ${prefix}buylimit dan dengan naik level`, text,{ quoted: mek})
		...
		for (let x of mentionUser) {
            ...
            if (afk.hasOwnProperty(sender.split('@')[0])) {
                nuy.sendMessage(from, `${pushname} Anda telah keluar dari mode afk`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
      ...
      // Tebak Gambar
		...
		if (isGroupAdmins) return nuy.sendMessage(from, 'Admin Grup Mah Bebas:v',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
		nuy.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return nuy.sendMessage(from, '#izinadmin diterima',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		nuy.sendMessage(from, `*_「 Link Terdeteksi 」_*\nHei *${pushname}*  *「 LINK TERDETEKSI ⌋ MAFF SEKARANG KAMU SAYA KICK DARI GRUP INI*`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
		setTimeout( () => {
			nuy.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 3000)
		setTimeout( () => {
			nuy.updatePresence(from, Presence.composing)
			nuy.sendMessage(from, 'SEEE YUO KACK;)',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
		}, 0)
	  }
      ...
      nuy.sendMessage(from, loli, MessageType.audio, {quoted: {key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "productMessage":{"product":{"productImage":{"mimetype":"image/jpeg","height":736,"width":736,"jpegThumbnail":fs.readFileSync(`./pinky.jpg`)},"productId":"3937202479680283","title":"Waalaikumsalam","currencyCode":"IDR","priceAmount1000":"9999","productImageCount":1},"businessOwnerJid":"0@s.whatsapp.net"}}}, mimetype: 'audio/mp4', ptt:true, duration:999999999999})
      }
      if (messagesC.includes('manca')){
	  nuy.updatePresence(from, Presence.composing)
	  nuy.sendMessage(from, `Oke Online...\nPrefix : | ${prefix} |\nSpeed : ${process.uptime()}`,MessageType.text, { quoted: ftroli, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
	  }
	  if (messagesC.includes('tes')){
	  nuy.updatePresence(from, Presence.composing)
	  nuy.sendMessage(from, `Work...?\nPrefix : | ${prefix} |\nSpeed : ${process.uptime()}`,MessageType.text, { quoted: {key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: '0@s.whatsapp.net' } : {}) }, message: {imageMessage: {url: 'https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc', mimetype: 'image/jpeg', caption: fakereply, fileSha256: '+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=', nfileLength: "28777", height: 1080, width: 1079, mediaKey: "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", fileEncSha256: "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", directPath: "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", mediaKeyTimestamp: "1610993486", jpegThumbnail: fs.readFileSync('pinky.jpg')} } }, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
	  }
	  if (messagesC.includes('@6283815956151')){
	  nuy.updatePresence(from, Presence.composing)
	  nuy.sendMessage(from, 'Ada Apa Tag Owner Manca...?',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
	  }
      ...
      nuy.sendMessage(from, loli, MessageType.audio, {quoted: {key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "productMessage":{"product":{"productImage":{"mimetype":"image/jpeg","height":736,"width":736,"jpegThumbnail":fs.readFileSync(`./pinky.jpg`)},"productId":"3937202479680283","title":"Astaghfirullah","currencyCode":"IDR","priceAmount1000":"9999","productImageCount":1},"businessOwnerJid":"0@s.whatsapp.net"}}}, mimetype: 'audio/mp4', ptt:true, duration:999999999999})
      }
      if (messagesC.includes('manca')){
      nuy.updatePresence(from, Presence.composing)
      ...
      nuy.sendMessage(from, loli, MessageType.audio, {quoted: {key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "productMessage":{"product":{"productImage":{"mimetype":"image/jpeg","height":736,"width":736,"jpegThumbnail":fs.readFileSync(`./pinky.jpg`)},"productId":"3937202479680283","title":"Sound Gabut","currencyCode":"IDR","priceAmount1000":"9999","productImageCount":1},"businessOwnerJid":"0@s.whatsapp.net"}}}, mimetype: 'audio/mp4', ptt:true, duration:999999999999})
      const d = fs.readFileSync('./sticker/joget.webp')
      nuy.sendMessage(from, d, sticker, {quoted: {key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "productMessage":{"product":{"productImage":{"mimetype":"image/jpeg","height":736,"width":736,"jpegThumbnail":fs.readFileSync(`./pinky.jpg`)},"productId":"3937202479680283","title":"Goyang Azek","currencyCode":"IDR","priceAmount1000":"9999","productImageCount":1},"businessOwnerJid":"0@s.whatsapp.net"}}}})
      }
      ...
      nuy.sendMessage(from, loli, MessageType.audio, {quoted: {key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "productMessage":{"product":{"productImage":{"mimetype":"image/jpeg","height":736,"width":736,"jpegThumbnail":fs.readFileSync(`./pinky.jpg`)},"productId":"3937202479680283","title":"Selamat Malam","currencyCode":"IDR","priceAmount1000":"9999","productImageCount":1},"businessOwnerJid":"0@s.whatsapp.net"}}}, mimetype: 'audio/mp4', ptt:true, duration:999999999999})
      }
      if (messagesC.includes('pagi')){
      nuy.updatePresence(from, Presence.composing)
      const loli = fs.readFileSync('./mp3/pagi.mp3')
      nuy.sendMessage(from, loli, MessageType.audio, {quoted: {key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "productMessage":{"product":{"productImage":{"mimetype":"image/jpeg","height":736,"width":736,"jpegThumbnail":fs.readFileSync(`./pinky.jpg`)},"productId":"3937202479680283","title":"Selamat Pagi","currencyCode":"IDR","priceAmount1000":"9999","productImageCount":1},"businessOwnerJid":"0@s.whatsapp.net"}}}, mimetype: 'audio/mp4', ptt:true, duration:999999999999})
      }
      ...
      nuy.sendMessage(from, loli, MessageType.audio, {quoted: {key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "productMessage":{"product":{"productImage":{"mimetype":"image/jpeg","height":736,"width":736,"jpegThumbnail":fs.readFileSync(`./pinky.jpg`)},"productId":"3937202479680283","title":"Iyah Sama Dua:v","currencyCode":"IDR","priceAmount1000":"9999","productImageCount":1},"businessOwnerJid":"0@s.whatsapp.net"}}}, mimetype: 'audio/mp4', ptt:true, duration:999999999999})
      }
      ...
      if (budy.startsWith('>')){
      if (!isOwner) return nuy.sendMessage(from, '「 OWNER BOT ONLY 」',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
      try {
      ...
      await reply(m)
			...
			switch(command) {
                ...
                if (args.length < 1) return nuy.sendMessage(from, 'on untuk menyalakan, off untuk mematikan',MessageType.text, { quoted: fvideo, contextInfo: { forwardingScore: 508, isForwarded: true}})
                if (args[0] === 'on') {
                if (isBadWord) return nuy.sendMessage(from, '*Fitur BadWord sudah aktif sebelum nya*',MessageType.text, { quoted: flokasi, contextInfo: { forwardingScore: 508, isForwarded: true}})
                 	   ...
                 	   fs.writeFileSync('./database/kelompok/badword.json', JSON.stringify(badword))
                  	   nuy.sendMessage(from, 'Sukses aktifkan Antibadword',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
              	  } else if (args[0] === 'off') {
                  	  badword.splice(from, 1)
                 	   fs.writeFileSync('./database/kelompok/badword.json', JSON.stringify(badword))
                 	    nuy.sendMessage(from, 'Sukses matikan Antibadword',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
             	   } else {
                 	   reply(ind.satukos())
                	}
                    break
...
nuy.sendMessage(from, { "degreesLatitude": 37.38980827772353,
            ...
            "name": namaxx,
            "address": jlnxx,
            "jpegThumbnail": fakeimagecuy }, location, { quoted:{
				  key: {
				   fromMe: false,
...
╚╝──────────╚═╝`, MessageType.extendedText,{
 quoted: {
  ...
  message: {
    ...
    surface: 10,
    message: 'Pinky Cans',
    orderTitle: 'PinkyCa', // Idk what this does
    sellerJid: '0@s.whatsapp.net' // Seller
   }
  }
 }
...
nuy.toggleDisappearingMessages(jid, WA_DEFAULT_EPHEMERAL)
}}	
...
break
		case 'sewabot1':
                ...
                if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
                    ...
                    nuy.sendMessage(from, 'Anda Telah Membeli Script!, tunggu owner script menchat anda!.\nUntuk Membatalkan Pesanan Ketik #batal1',MessageType.text, { quoted: fgclink, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
                    break
		case 'batal1':
				if (isBanned) return reply(ind.baned())
                if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
                    ...
                    nuy.sendMessage('6283815956151@s.whatsapp.net', options, text, {quoted: mek})
                    nuy.sendMessage(from, 'Anda Telah Membatalkan Pesanan!.',MessageType.text, { quoted: fdocs, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
                    break
                    case 'batal2':
				if (isBanned) return reply(ind.baned())
                if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
		...
		case 'sewabot2':
                ...
                if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
                    ...
                    break
		case'daftarsewa':
sewa = fs.readFileSync('pinky.jpg')
...
${prefix}sewa1
----------------------------------------------------`
nuy.sendMessage(from, sewa, image, {quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true, mentionedJid: [ow, ow2, ow3, ow4]}, displayname: mem, caption: daftarnye })
break
		case 'mute':
            ...
            case 'unmute':
                ...
                break
        case 'tomp3':
            if (!isQuotedVideo) return nuy.sendMessage(from, 'Reply videonya!',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
            nuy.sendMessage(from, 'Sedang Diproses...',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
            encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo
            media = await nuy.downloadAndSaveMediaMessage(encmedia)
            ran = getRandom('.mp4')
            exec(`ffmpeg -i ${media} ${ran}`, (err) => {
            fs.unlinkSync(media)
            if (err) return nuy.sendMessage(from, 'Error!',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
		...
		case 'chat':
	       ...
	       case 'join':
               ...
               nuy.sendMessage(from, 'Success Gabung Group',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
               }, 20000) // 1000 = 1s,
               setTimeout( () => {
               nuy.sendMessage(from, 'Oke Owner 🤗',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
               }, 0) // 1000 = 1s,
               break
          case 'circle':
               ...
               if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
                ...
                nuy.sendMessage(from, hedhe, sticker, {quoted: mek, contextInfo: {"forwardingScore": 999, "isForwarded": true}})
               } else {
                 reply('Hamdeh')
               }
               break
					...
					if (!isOwner) return nuy.sendMessage(from, '「 OWNER BOT ONLY 」',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					LoLKey = args.join(" ")
					nuy.sendMessage(from, `LoLKey berhasil di ubah menjadi : ${LoLKey}`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
					break 
					case 'settobz':
					if (!isOwner) return nuy.sendMessage(from, '「 OWNER BOT ONLY 」',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					TobzKey = args.join(" ")
					nuy.sendMessage(from, `TobzKey berhasil di ubah menjadi : ${TobzKey}`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
					break 
					case 'setxteam':
					if (!isOwner) return nuy.sendMessage(from, '「 OWNER BOT ONLY 」',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					XteamKey = args.join(" ")
					nuy.sendMessage(from, `XteamKey berhasil di ubah menjadi : ${XteamKey}`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
					break 
				    case 'setwmsticker':
					if (!isOwner) return nuy.sendMessage(from, '「 OWNER BOT ONLY 」',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					...
					nuy.sendMessage(from, 'Namo Dan Ator berhasil di ubah',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					break 
					case 'setfakereply':
					if (!isOwner) return nuy.sendMessage(from, '「 OWNER BOT ONLY 」',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					fakereply = args.join(" ")
					nuy.sendMessage(from, `Fakereply berhasil di ubah menjadi : ${fakereply}`,MessageType.text, { quoted: flokasi, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					break 
					case 'setfakereply1':
					if (!isOwner) return nuy.sendMessage(from, '「 OWNER BOT ONLY 」',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					fakereply1 = args.join(" ")
					nuy.sendMessage(from, `Fakereply1 berhasil di ubah menjadi : ${fakereply1}`,MessageType.text, { quoted: fvideo, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					break 
				case 'fuckboy':
				case 'fuckgirl':
				case 'bucinserti':
		...
		if (!isGroup) return reply(ind.groupo())
					...
					setTimeout( () => {
					nuy.sendMessage(from, '*➸ Jawaban :* '+anu.result.answer, text, {quoted: mek}) // ur cods
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
					...
					setTimeout( () => {
					nuy.sendMessage(from, jenak, text, {quoted: mek }) // ur cods
					}, 0) // 1000 = 1s,
					await limitAdd(sender) 
					break 
		case 'tebakgokil2':
		        ...
		        if (!isGroup) return reply(ind.groupo())
					...
					setTimeout( () => {
					nuy.sendMessage(from, siapariu, text, {quoted: mek }) // ur cods
					}, 0) // 1000 = 1s,
					await limitAdd(sender) 
					break 
		case 'caklontong2':
...
*➸ Info :* ${anu.result.information}`
					...
					setTimeout( () => {
					nuy.sendMessage(from, '_10 Detik lagi…_', MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}}) // ur cods
					}, 20000) // 1000 = 1s,
					setTimeout( () => {
					nuy.sendMessage(from, '_20 Detik lagi_…', MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}}) // ur cods
					}, 10000) // 1000 = 1s,
					setTimeout( () => {
					nuy.sendMessage(from, '_30 Detik lagi_…', MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}}) // ur cods
					}, 2500) // 1000 = 1s,
					setTimeout( () => {
		...
		if (!isGroup) return reply(ind.groupo())
					...
					setTimeout( () => {
					nuy.sendMessage(from, '*➸ Jawaban :* '+anu.result.lambang, text, {quoted: mek}) // ur cods
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
					nuy.sendMessage(from, '_10 Detik lagi…_', text) // ur cods
					}, 20000) // 1000 = 1s,
					...
					setTimeout( () => {
					nuy.sendMessage(from, kimia, text, {quoted: mek }) // ur cods
					}, 0) // 1000 = 1s,
					await limitAdd(sender) 
					break 
		case 'tebaklirik':
		...
		if (!isGroup) return reply(ind.groupo())
					...
					setTimeout( () => {
					nuy.sendMessage(from, lirik, text, {quoted: mek }) // ur cods
					}, 0) // 1000 = 1s,
					await limitAdd(sender) 
					break 
		case 'ghost':
		           if (!isRegistered) return reply(ind.noregis())
					...
					nuy.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', quoted: mek})
                    fs.unlinkSync(ran)
				    })
		            break
		case 'tempo-v':
   var req = args.join(' ')            
					...
					exec(`ffmpeg -i ${media} -filter:a "atempo=1.0,asetrate=${req}" ${ran}`, (err, stderr, stdout) => {
						...
						if (err) return reply('Error!')
						hah = fs.readFileSync(ran)
						nuy.sendMessage(from, hah, video, { mimetype: 'video/mp4', quoted: mek })
})
				break
				case 'spam':
				argz = args.split("|")
...
var req = args.join(' ')            
				...
				case 'gemes':
					...
					nuy.sendMessage(from, man, audio, {mimetype: 'audio/mp4', ptt:true, quoted: mek})
						fs.unlinkSync(ran)
					})
					break
						case 'stickermeme':
						...
						//nuy.sendMessage(from, buffer, MessageType.sticker, {quoted: mek})
							break
           case 'trigger2':
               if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					if (isBanned) return nuy.sendMessage(from, `Maaf ${pushname} kamu sudah terbenned!`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
                  ...
                  nuy.sendMessage(from, 'Gunakan Foto...!',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
               }
               break
        case 'trigger':
					if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
       ...
       exec(`wget ${anu1} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
		...
		if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
		   if (isBanned) return nuy.sendMessage(from, `Maaf ${pushname} kamu sudah terbenned!`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
                    ...
                    nuy.sendMessage(from, thumbnail, image, { quoted: mek, caption: ini_txt })
                    break
                case 'wait2':
		if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
           if (isBanned) return nuy.sendMessage(from, `Maaf ${pushname} kamu sudah terbenned!`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
                    if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
                        ...
                        nuy.sendMessage(from, ini_video, video, { quoted: mek, caption: ini_txt })
                    } else {
        ...
        case 'pinkyy':
            if (!isOwner) return nuy.sendMessage(from, '「 OWNER BOT ONLY 」',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
            await nuy.GoogleDispenserMessages(from,`mampus lag`,text)
			reply("Pinky Neh")
			break
		case 'hetshot':
			if (!isGroup) return reply(ind.groupo())
			...
			nuy.sendMessage(from, 'SUKSES',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
			break
		case 'setthumb':
		        if (!isOwner) return nuy.sendMessage(from, '「 OWNER BOT ONLY 」',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				boij = JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo
				delb = await nuy.downloadMediaMessage(boij)
				fs.writeFileSync(`pinky.jpg`, delb)
				nuy.sendMessage(from, 'Sukses Beb😉',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
				break
	    case 'setthumbb':
	            if (!isAdmin) return nuy.sendMessage(from, 'Only Admin Bot',MessageType.text, { quoted: ftroli, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				boij = JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo
				delb = await nuy.downloadMediaMessage(boij)
				fs.writeFileSync(`pinky.jpg`, delb)
		...
		case 'harcos':
					...
					cap = `${query}`
					nuy.sendMessage(from, buffer, image, { quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true}, caption: cap})
					break
		case 'harta':
				if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				if (isBanned) return nuy.sendMessage(from, `Maaf ${pushname} kamu sudah terbenned!`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				...
				nuy.sendMessage(from, buffer, image, {quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, caption: cap })
				await limitAdd(sender)
				break
		case 'setthumb':
				...
				nuy.sendMessage(from, `\`\`\`Sukses Mengganti Thumbnail\`\`\``, MessageType.text, { quoted: mek2 })
				break
		case 'togif':
                                        ...
                                        nuy.sendMessage(from, buff, video, { mimetype: Mimetype.gif, caption: 'Oke...', quoted: mek})
                                }
                                await limitAdd(sender)
                                break
		case 'getpic':
		...
		case 'tovideo':
					...
					exec(`ffmpeg -i ${anum} ${ran}`, (err) => {
						...
						nuy.sendMessage(from, buffers, video, { quoted: mek2, caption: 'Sudah...' })
						fs.unlinkSync(ran)
					})
					break
		case 'kontak':
		...
		case 'upswtext':
					...
					nuy.sendMessage(from, `Sukses Up story wea teks ${q}`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
					break
		...
		case 'kontag2':
					...
					const vcardtag = 'BEGIN:VCARD\n'
					            ...
					            + 'END:VCARD'
            				nuy.sendMessage(from, {displayname: mem, vcard: vcardtag}, MessageType.contact, { quoted: mek, contextInfo: {mentionedJid: mem}, quoted: mek })
				        break
        case 'kontag':
                    if (!isGroup) return reply(ind.groupo())
		            if (!isGroupAdmins) return reply(ind.admin())
                    if (args.length < 1) return nuy.sendMessage(from, `cara : ${prefix + command} Saha|6283815956151`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
                    ...
                    nuy.sendMessage(from, {displayname: nuy, vcard: vcard4}, MessageType.contact, {quoted: mek, contextInfo: {mentionedJid: mem}})
                    break
				case 'tempo':
				nuy.sendMessage(from, 'Sedang Diproses...',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
                    var req = args.join(" ")            
					...
					exec(`ffmpeg -i ${media} -filter:a "atempo=1.0,asetrate=${req}" ${ran}`, (err, stderr, stdout) => {
						...
						if (err) return reply('Error!')
						hah = fs.readFileSync(ran)
						nuy.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, quoted: mek, duration:999999999999})
						fs.unlinkSync(ran)
					})
				break
		case 'tagimg':
        ...
        case 'totag':
          ...
          nuy.sendMessage(from, `reply gambar/sticker/audio/video dengan caption ${prefix}mctag`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
        }
        break
		case 'removebg':
	if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
	...
	if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
	 ...
	 nuy.sendMessage(from, hehe, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "productMessage":{"product":{"productImage":{"mimetype":"image/jpeg","height":736,"width":736,"jpegThumbnail":fs.readFileSync(`./pinky.jpg`)},"productId":"3937202479680283","title":"MANCA BOT","currencyCode":"IDR","priceAmount1000":"9999","productImageCount":1},"businessOwnerJid":"0@s.whatsapp.net"}}}, contextInfo: { forwardingScore: 508, isForwarded: true}, caption: 'Nih Dah Jadi...\n\n*Follow!!* @thenay_xploit_'})
	} else {
	  nuy.sendMessage(from, 'Reply Imagenya!!',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
	}
	break
	case 'smile':
	if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
	...
	break
		case 'profile':
					...
					nuy.sendMessage(from, buff, image, {quoted: mek2, caption: profile})
					break
		case 'afk':
                    alasan = args.join(" ")
...
`
            reply(brow)
	            break
				case 'shutdown':
                nuy.sendMessage(from, 'Oke Beb...',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
                await sleep(5000)
		return nuy.sendMessage(from, JSON.stringify(eval(process.exit())), text, {quoted: mek})
				break
			case 'slot2':
		const sotoy = [
		'🍊 : 🍒 : 🍐',
...
_> ${get_result.hasil}_`
				...
				case 'nightcore':
					...
					exec(`ffmpeg -i ${media} -filter:a atempo=1.06,asetrate=44100*1.25 ${ran}`, (err, stderr, stdout) => {
						...
						if (err) return reply('Error!')
						hah = fs.readFileSync(ran)
						nuy.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:false, quoted: mek2,duration:999999999999})
						fs.unlinkSync(ran)
					   })
				       break
			case 'slow':
			       nuy.sendMessage(from, 'Sedang Diproses...',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
					...
					exec(`ffmpeg -i ${media} -filter:a "atempo=0.7,asetrate=44100" ${ran}`, (err, stderr, stdout) => {
						...
						if (err) return reply('Error!')
						hah = fs.readFileSync(ran)
						nuy.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, quoted: mek2, duration:999999999999})
						fs.unlinkSync(ran)
					})
				break
				case 'gemuk':
              ...
              case 'tupai':
				...
				nuy.sendMessage(from, uhh, audio, {mimetype: 'audio/mp4', ptt:true, quoted: mek2, duration:999999999999})
				fs.unlinkSync(ran)
				})
				break
				case 'addbadword': 
                    if (!isOwner) return nuy.sendMessage(from, '「 OWNER BOT ONLY 」',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
                    if (!isGroupAdmins) return reply(ind.admin())
                    if (args.length < 1) return nuy.sendMessage(from, `Kirim perintah ${prefix}addbadword [kata kasar]. contoh ${prefix}addbadword bego`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
                    const bw = body.slice(12)
                    bad.push(bw)
                    fs.writeFileSync('./database/kelompok/bad.json', JSON.stringify(bad))
                    nuy.sendMessage(from, 'Success Menambahkan Bad Word!',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
                    break
                case 'delbadword': 
                    if (!isOwner) return nuy.sendMessage(from, '「 OWNER BOT ONLY 」',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
                    if (!isGroupAdmins) return reply(ind.admin())
                    if (args.length < 1) return nuy.sendMessage(from, `Kirim perintah ${prefix}delbadword [kata kasar]. contoh ${prefix}delbadword bego`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
                    let dbw = body.slice(12)
                    bad.splice(dbw)
                    fs.writeFileSync('./database/kelompok/bad.json', JSON.stringify(bad))
                    nuy.sendMessage(from, 'Success Menghapus BAD WORD!',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
                    break 
                case 'listbadword': 
				...
				if (argz[1] == 'aktif') {
					if (isGroup) {
						if (isRevoke) return nuy.sendMessage(from, `Antidelete telah diaktifkan di grup ini sebelumnya!`, MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
						dataRevoke.push(from)
						fs.writeFileSync('./src/gc-revoked.json', JSON.stringify(dataRevoke, null, 2))
						nuy.sendMessage(from, `*Succes Enable Antidelete Grup!*`, MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
					} else if (!isGroup) {
						nuy.sendMessage(from, `Untuk kontak penggunaan *${prefix}antidelete ctaktif*`, MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
					}
				} else if (argz[1] == 'ctaktif') {
					if (!isGroup) {
						if (isCtRevoke) return nuy.sendMessage(from, `Antidelete telah diaktifkan di semua kontak sebelumnya!`, MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
						dataCtRevoke.data = true
						fs.writeFileSync('./src/ct-revoked.json', JSON.stringify(dataCtRevoke, null, 2))
						nuy.sendMessage(from, `Antidelete diaktifkan disemua kontak!`, MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
					} else if (isGroup) {
						nuy.sendMessage(from, `Untuk grup penggunaan *${prefix}antidelete aktif*`, MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
					}
				} else if (argz[1] == 'banct') {
					if (isBanCtRevoke) return nuy.sendMessage(from, `kontak ini telah ada di database banlist!`, MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
					if (argz.length === 2 || argz[2].startsWith('0')) return nuy.sendMessage(from, `Masukan nomer diawali dengan 62! contoh 62838159xxxxx`, MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
					dataBanCtRevoke.push(argz[2] + '@s.whatsapp.net')
					fs.writeFileSync('./src/ct-revoked-banlist.json', JSON.stringify(dataBanCtRevoke, null, 2))
					nuy.sendMessage(from, `Kontak ${argz[2]} telah dimasukan ke banlist antidelete secara permanen!`, MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
				} else if (argz[1] == 'mati') {
					if (isGroup) {
						const index = dataRevoke.indexOf(from)
						dataRevoke.splice(index, 1)
						fs.writeFileSync('./src/gc-revoked.json', JSON.stringify(dataRevoke, null, 2))
						nuy.sendMessage(from, `*Succes disable Antidelete Grup!*`, MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
					} else if (!isGroup) {
						nuy.sendMessage(from, `Untuk kontak penggunaan *${prefix}antidelete ctmati*`, MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
					}
				} else if (argz[1] == 'ctmati') {
					if (!isGroup) {
						dataCtRevoke.data = false
						fs.writeFileSync('./src/ct-revoked.json', JSON.stringify(dataCtRevoke, null, 2))
						nuy.sendMessage(from, `Antidelete dimatikan disemua kontak!`, MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
					} else if (isGroup) {
						nuy.sendMessage(from, `Untuk grup penggunaan *${prefix}antidelete mati*`, MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
					}
				}
				break
	case 'alien':
	...
	break
		...
		case 'next':
				...
				nuy.sendMessage(from, `Berhasil menambahkan admin bot wa.me/${admm}`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
				break
                case 'unadmin':
				if (!isOwner) return nuy.sendMessage(from, '「 OWNER BOT ONLY 」',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				admm = body.slice(9)
				adm.push(`${admm}@s.whatsapp.net`)
				fs.writeFileSync('./database/pengguna/admin.json', JSON.stringify(adm))
				nuy.sendMessage(from, `Berhasil Mengunadmin bot wa.me/${admm}`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
				break
                case 'wakillist':
				nuy.updatePresence(from, Presence.composing) 
                 if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})    
				...
				nuy.sendMessage(from, teks.trim(), extendedText, {quoted: mek2, contextInfo: {"mentionedJid": adm}})
				break
                case 'premium':
				if (!isOwner) return nuy.sendMessage(from, '「 OWNER BOT ONLY 」',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				premm = body.slice(9)
				...
				nuy.sendMessage(from, `Berhasil menjadi premium wa.me/${premm}`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
				break
		case 'unpremium':
				if (!isOwner) return nuy.sendMessage(from, '「 OWNER BOT ONLY 」',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				premm = body.slice(11)
				...
				nuy.sendMessage(from, `Nomor sudah berakhir menjadi premium wa.me/${premm}`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
				break
                case 'premiumlist':
				nuy.updatePresence(from, Presence.composing) 
				...
				nuy.sendMessage(from, teks.trim(), extendedText, {quoted: mek2, contextInfo: {"mentionedJid": prem}})
				break
                case 'bann':
				if (!isAdmin) return nuy.sendMessage(from, 'Only Admin Bot',MessageType.text, { quoted: ftroli, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				bnnd = body.slice(5)
				...
				nuy.sendMessage(from, `Berhasil membanned nomor : wa.me/${bnnd} `,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
				break
		case 'unbann':
				if (!isAdmin) return nuy.sendMessage(from, 'Only Admin Bot',MessageType.text, { quoted: ftroli, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				bnnd = body.slice(7)
				...
				fs.writeFileSync('./database/pengguna/banned.json', JSON.stringify(ban))
				nuy.sendMessage(from, `Nomor wa.me/${bnnd} telah di unban!`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
				break
                case 'ban':
				if (!isOwner) return nuy.sendMessage(from, '「 OWNER BOT ONLY 」',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				bnnd = body.slice(5)
		...
		case 'unban':
				...
				nuy.sendMessage(from, teks.trim(), extendedText, {quoted: mek2, contextInfo: {"mentionedJid": ban}})
				break
		...
		case 'giftlimit': 
                 ...
                 if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				if (!isAdmin) return nuy.sendMessage(from, 'Only Adminban & Owner Kami!',MessageType.text, { quoted: ftroli, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
                		...
                		var found = false
							...
							*「 GIFT KUOTA LIMIT 」*
							...
							• Limit: ${limitawal-updated.limit}`
                                ...
                                nuy.sendMessage(from, `Maaf, nomor ${nomerr} tidak terdaftar di database!`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
                        	}
               			break
                case 'moddroid':
                 if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				...
				nuy.sendMessage(from, buffer, image, {quoted: mek2, caption: `${teks}`})
				await limitAdd(sender)
				break
		case 'happymod':
                 if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
		...
		case 'cuttly':
				...
				if (isBanned) return nuy.sendMessage(from, `Maaf ${pushname} kamu sudah terbenned!`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					...
					nuy.sendMessage(from, alan, image, {quoted: mek2})
					await limitAdd(sender)
				break
				case 'googletext':
                    if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				if (isBanned) return nuy.sendMessage(from, `Maaf ${pushname} kamu sudah terbenned!`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					...
					nuy.sendMessage(from, uwuw, image, {quoted: mek2})
					await limitAdd(sender)
				break
		case 'tinyurl':
		       if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
		...
		case 'shrtco':
				...
				if (isBanned) return nuy.sendMessage(from, `Maaf ${pushname} kamu sudah terbenned!`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					...
					exec(`wget ${anu.url} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						...
						nuy.sendMessage(from, buffer, sticker, {quoted: mek2})
						fs.unlinkSync(rano)
					})
					await limitAdd(sender)
					break
		...
		case 'randomhentai':
				...
				if (isBanned) return nuy.sendMessage(from, `Maaf ${pushname} kamu sudah terbenned!`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					...
					nuy.sendMessage(from, 'Sedang Diproses...',MessageType.text, { quoted: fgif, contextInfo: { forwardingScore: 508, isForwarded: true}})
                                        ...
                                        nuy.sendMessage(from, buffer, image, {quoted: mek2})
					await limitAdd(sender)
                                        break
		        case 'art':
                case 'bts':
                ...
                case 'hentai4everyone':
                    ...
                    nuy.sendMessage(from, ini_buffer, image, { quoted: mek2})
                    break
                case 'bj':
                case 'ero':
                case 'cum':
		...
		if (isBanned) return nuy.sendMessage(from, `Maaf ${pushname} kamu sudah terbenned!`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
                    ...
                    exec(`wget ${ini_buffer} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
                        ...
                        nuy.sendMessage(from, buff, sticker, { quoted: mek2})
                        fs.unlinkSync(rano)
                    })
                    break
		case 'nulis':
				...
				nuy.sendMessage(from, ct, image, {caption: 'Nih kak udah jadi..', quoted: mek2})
				await limitAdd(sender)
				break
		case 'nulis3':
				...
				nuy.sendMessage(from, gools5, image, {caption: 'Nih kak udah jadi..', quoted: mek2})
				await limitAdd(sender)
				break
		case 'nulis2':
		...
		case 'bplogo':
				...
				buffer = await getBuffer(anu.result)
				nuy.sendMessage(from, buffer, image, {caption: 'Nih kak udah jadi..', quoted: mek2})
				await limitAdd(sender)
				break
		case 'harta':
                 if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				...
				nuy.sendMessage(from, taa, image, {caption: 'Nih kak udah jadi..', quoted: mek2})
				await limitAdd(sender)
				break
		case 'tahta':
                 if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				...
				nuy.sendMessage(from, bh, image, {caption: 'Nih kak udah jadi..', quoted: mek2})
				await limitAdd(sender)
				break
        case '1977':
        case 'aden':
        ...
        if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
		if (isBanned) return nuy.sendMessage(from, `Maaf ${pushname} kamu sudah terbenned!`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
        nuy.sendMessage(from, 'Sedang Diproses...',MessageType.text, { quoted: fgif, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
        var imgbb = require('imgbb-uploader')
        if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
        encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo: mek
        media = await nuy.downloadAndSaveMediaMessage(encmedia)
		...
		case 'darkneon':
            ...
            case 'shadow2':
                ...
                case 'carvedwood':  // Fix Bug By MancaBot & Nurul
                    if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				    if (isBanned) return nuy.sendMessage(from, `Maaf ${pushname} kamu sudah terbenned!`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
                    if (args.length == 0) return nuy.sendMessage(from, 'Teksnya Mana Kak?',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
                    nuy.sendMessage(from, 'Sedang Diproses...',MessageType.text, { quoted: fgif, contextInfo: { forwardingScore: 508, isForwarded: true}})
                    txt = args.join(" ")
            ...
            case 'blackpink':
				...
				if (isBanned) return nuy.sendMessage(from, `Maaf ${pushname} kamu sudah terbenned!`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				nuy.sendMessage(from, 'Sedang Diproses...',MessageType.text, { quoted: fgif, contextInfo: { forwardingScore: 508, isForwarded: true}})
                    if (args.length == 0) return nuy.sendMessage(from, `Example: ${prefix + command} Manca`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
                    txt = args.join(" ")
	...
	case 'emottoimg':
		...
		case 'calendermaker':
                 if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				if (isBanned) return nuy.sendMessage(from, `Maaf ${pushname} kamu sudah terbenned!`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				if (args.length < 1) return nuy.sendMessage(from, 'Url png/jpg mana kak',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
		...
		case 'shota':
				...
				if (isBanned) return nuy.sendMessage(from, `Maaf ${pushname} kamu sudah terbenned!`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				if (args.length < 1) return nuy.sendMessage(from, 'Teks nya mana kak?',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
				tels = body.slice(10)
				nuy.sendMessage(from, 'Sedang Diproses...',MessageType.text, { quoted: fgif, contextInfo: { forwardingScore: 508, isForwarded: true}})
				...
				case 'emoji2img': 
                 if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				if (isBanned) return nuy.sendMessage(from, `Maaf ${pushname} kamu sudah terbenned!`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				if (args.length < 1) return nuy.sendMessage(from, 'Emoji nya mana kak?',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
               ...
               nuy.sendMessage(from, asupan, video, { mimetype: 'video/mp4', quoted: {key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "productMessage":{"product":{"productImage":{"mimetype":"image/jpeg","height":736,"width":736,"jpegThumbnail":fs.readFileSync(`./pinky.jpg`)},"productId":"3937202479680283","title":"SEKEDAR UNTUK GABUT","currencyCode":"IDR","priceAmount1000":"9999","productImageCount":1},"businessOwnerJid":"0@s.whatsapp.net"}}}, caption: 'Nih kak asupan nya' })
               await limitAdd(sender)
               break
				case 'asupansantuy':
				case 'asupanbocil':
		...
		case 'aesthetic':
				...
				nuy.sendMessage(from, buffer, image, {quoted: mek2, caption: 'Nih aestheticnya kak...'})
				await limitAdd(sender)
				break
		case 'darklogo': 
		//Case By FxSx
				...
				nuy.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih Dah Jadi...\n\n*Follow!!* @thenay_xploit_'})
				await limitAdd(sender)
				break
				case 'ballon':
				//Case By FxSx
...
╰──────────`
                    ...
                    await nuy.sendMessage(from, thumbnail, image, { quoted: mek, caption: cuy, contextInfo: { forwardingScore: 1, isForwarded: true} })
                    get_audio = await getBuffer(get_result.audio[0].link)
                    await nuy.sendMessage(from, get_audio, audio, { mimetype: 'audio/mp4', filename: `${get_result.info.song}.mp3`, quoted: mek2, duration: 99999999999, contextInfo: { forwardingScore: 508, isForwarded: true, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')}})
                    break
          case 'play2':
		         if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				 if (isBanned) return nuy.sendMessage(from, `Maaf ${pushname} kamu sudah terbenned!`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				    ...
				    nuy.sendMessage(from, buffer, image, {quoted: mek2, caption: infomp3})
         ...
         case 'play':
                ...
                if (args.length == 0) return reply(`Contoh: ${prefix + command} Melukis Senja`)
                    ...
                    nuy.sendMessage(from, buffer, image, {quoted: mek2, caption: ini_txt})
		 ...
		 case 'vinta':
				...
				nuy.sendMessage(from, vintage, image, {caption: `Nih kak ${vin}`, quoted: mek2})
				await limitAdd(sender)
				break
                case '1cak':
				    try {
                 if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
						...
						nuy.sendMessage(from, buffer, image, {quoted: mek2, caption: 'Nih Kak udah Jadi'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(ind.wrongf())
					}
		...
		case 'infocuaca':
				...
				if (isBanned) return nuy.sendMessage(from, `Maaf ${pushname} kamu sudah terbenned!`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					...
					nuy.sendMessage(from, hasil, MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
					await limitAdd(sender)
					break
		case 'kurs': 
                 if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
		...
		case 'githubstalk':
					...
					nuy.sendMessage(from, buffer, image, {quoted: mek2, caption: txt})
					break
                case 'slap':
					...
					nuy.sendMessage(from, pod, image, { quoted: mek2, caption: '*Toxic*\n\n'+ ple })
					await limitAdd(sender)
					break
		case 'tampar':
                 if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
		...
		case 'setppbot':
					...
					nuy.sendMessage(from, 'Makasih profile barunya owner😉',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
					break 
		case 'setppbott':
				if (!isAdmin) return nuy.sendMessage(from, 'Only Admin Bot',MessageType.text, { quoted: ftroli, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					nuy.updatePresence(from, Presence.composing) 
					...
					media = await nuy.downloadAndSaveMediaMessage(enmedia)
					await nuy.updateProfilePicture(botNumber, media)
					nuy.sendMessage(from, 'Makasih Profile Barunya Wakil Owner😉',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
					break 
		case 'brainly':
				...
				if (isBanned) return nuy.sendMessage(from, `Maaf ${pushname} kamu sudah terbenned!`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					...
					nuy.sendMessage(from, teks, text, {quoted: mek2, detectLinks: false})
					console.log(res)
					})
					await limitAdd(sender)
					break 
		case 'bcgc':
					if (!isOwner) return nuy.sendMessage(from, '「 OWNER BOT ONLY 」',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					if (args.length < 1) return nuy.sendMessage(from, 'Text Nya Mana Owner 🙄',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')}})
		...
		case 'img2':
           ...
           case 'glitchtext':
					...
					var gli = gh.split("/")[0];
					var tch = gh.split("/")[1];
					if (args.length < 1) return nuy.sendMessage(from, `Contoh : ${prefix}glitchtext Nuy/Cans`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
					nuy.sendMessage(from, 'Sedang Diproses...',MessageType.text, { quoted: fgif, contextInfo: { forwardingScore: 508, isForwarded: true}})
		...
		break
                ...
                case 'tebakgambar': 
                    ...
                    await nuy.sendMessage(from, ini_buffer, image, { quoted: mek, caption: "Jawab gk? Jawab lahhh, masa enggak. 30 detik cukup kan? gk cukup pulang aja" }).then(() => {
                    ...
                    break
                case 'canceltebak':
                    if (!tebakgambar.hasOwnProperty(sender.split('@')[0])) return nuy.sendMessage(from, 'Anda tidak memiliki tebak gambar sebelumnya',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
                    delete tebakgambar[sender.split('@')[0]]
                    fs.writeFileSync("./database/tebakgambar.json", JSON.stringify(tebakgambar))
                    nuy.sendMessage(from, 'Success mengcancel tebak gambar sebelumnya',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
                    break
				case 'family100':
					if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
		...
		case 'spamcall':
			...
			nuy.sendMessage(from, `${anu.result.logs}`, MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
			break  
				case 'wiki':
		if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
                    if (args.length == 0) return nuy.sendMessage(from, `Example: ${prefix + command} Tahu`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
                    query = args.join(" ")
               ...
               case 'ytmp4':
		if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
                    if (args.length == 0) return nuy.sendMessage(from, `Example: ${prefix + command} https://www.youtube.com/`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
                    ini_link = args[0]
...
─────────────────────`
                    ...
                    nuy.sendMessage(from, ini_buffer, image, { quoted: mek, caption: asu2, contextInfo: { forwardingScore: 2, isForwarded: true} })
                    get_audio = await getBuffer(get_result.link[0].link)
                    nuy.sendMessage(from, get_audio, video, { mimetype: 'video/mp4', filename: `${get_result.title}.mp4`, quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, duration:999999999999})
                    break
			case 'ytmp3':
		if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
...
─────────────────────`
                    ini_buffer = await getBuffer(get_result.thumbnail)
                    nuy.sendMessage(from, ini_buffer, image, { quoted: mek, caption: asu, contextInfo: { forwardingScore: 508, isForwarded: true}})
                    get_audio = await getBuffer(get_result.link[3].link)
                    nuy.sendMessage(from, get_audio, audio, { mimetype: 'audio/mp4', filename: `${get_result.title}.mp3`, quoted: mek, duration: 99999999999, contextInfo: { forwardingScore: 508, isForwarded: true}})
                    break
		case 'hilih': 
					
                 if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				if (isBanned) return nuy.sendMessage(from, `Maaf ${pushname} kamu sudah terbenned!`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					if (args.length < 1) return nuy.sendMessage(from, 'kasih teks lah^_^!!!',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
		...
		case 'chord':
                 if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				if (isBanned) return nuy.sendMessage(from, `Maaf ${pushname} kamu sudah terbenned!`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					if (args.length < 1) return nuy.sendMessage(from, 'Judul lagunya mana kak',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
					tels = body.slice(7)					
       ...
       if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
          ...
          options = {
            ...
            nuy.sendMessage(from, options, MessageType.text, {quoted: mek2})
           breaki
		case 'cerpen': 
		...
		case 'heroml':  
		       if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				...
				case 'igvideo': 
                 if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				if (args.length < 1) return nuy.sendMessage(from, 'Urlnya mana kak?',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
				if (!isUrl(args[0]) && !args[0].includes('www.instagram.com')) return reply(mess.error.lv)
					...
					if (anu.error) return reply(anu.error)
					buffer = await getBuffer(anu.result)
                    nuy.sendMessage(from, buffer, video, {quoted: mek2, duration:999999999999})
				    break
				case 'igpost': 
                 if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				if (args.length < 1) return nuy.sendMessage(from, 'Urlnya mana kak?',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
				...
				if (!isUrl(args[0]) && !args[0].includes('www.instagram.com')) return reply(mess.error.lv)
					...
					if (anu.error) return reply(anu.error)
					buffer = await getBuffer(anu.result)
                    nuy.sendMessage(from, buffer, video, {quoted: mek2})
				    break
				case 'tiktok': 
                 if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				...
				if (!isUrl(args[0]) && !args[0].includes('tiktok.com')) return reply(mess.error.lv)
                 ...
                 case 'tiktokmusic':
                   ...
                   nuy.sendMessage(from, 'Sedang Diproses...',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
                    ...
                    nuy.sendMessage(from, ini_buffer, audio, {mimetype: 'audio/mp4', quoted: fgif})
                    break
				 case 'ccgenerator': 
					if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
                   nuy.sendMessage(from, 'Sedang Diproses...',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
				   ...
				   nuy.sendMessage(from, teks, MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
				   await limitAdd(sender)
				   break
				 case 'ceritahorror': 
					if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
		...
		case 'setreply':
					...
					cr = body.slice(10)
					nuy.sendMessage(from, `reply berhasil di ubah menjadi : ${cr}`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
					await limitAdd(sender)
					break 
		case 'setreplyy':
				if (!isAdmin) return nuy.sendMessage(from, 'Only Admin Bot',MessageType.text, { quoted: ftroli, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
		...
		case 'menu':
                 if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				if (isBanned) return nuy.sendMessage(from, 'Maaf kamu sudah terbenned!',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
...
❏ NuyFaa
╰──────────` 
buffer = await getBuffer('https://raw.githubusercontent.com/alonecans/foto-manca/main/pinky.jpg')
                    nuy.sendMessage(from, buffer, image, {quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true, mentionedJid: [num]}, displayname: mem, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64'), caption: menusu })
					break
                  case 'addstic':
                case 'adds':
				    if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					if (!isQuotedSticker) return nuy.sendMessage(from, 'Reply stiker nya',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
					svst = body.slice(12)
					if (!svst) return nuy.sendMessage(from, 'Nama sticker nya apa?',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
					boij = JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo
					delb = await nuy.downloadMediaMessage(boij)
					setiker.push(`${svst}`)
               ...
               case 'slist':
				...
				if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					...
					nuy.sendMessage(from, teks.trim(), extendedText, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true, contextInfo: { "mentionedJid": setiker } }})
					break
              case 'gs':
				case 'gets':
				if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					...
					nuy.sendMessage(from, result, sticker, {quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
					break
              case 'addvn':
				if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					if (!isQuotedAudio) return nuy.sendMessage(from, 'Reply Audio Nya',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
					svst = body.slice(7)
					if (!svst) return nuy.sendMessage(from, 'Nama audionya?',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
					boij = JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo
					delb = await nuy.downloadMediaMessage(boij)
					audionye.push(`${svst}`)
            ...
            case 'imglist':
				...
				if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					...
					nuy.sendMessage(from, teks.trim(), extendedText, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true, contextInfo: { "mentionedJid": imagenye } }})
					break
            case 'gimg':
            case 'getimg':
            if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					...
					nuy.sendMessage(from, buffer, image, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true, caption: `Result From Database : ${namastc}.jpeg` }})
					break
            case 'addvid':
				if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					if (!isQuotedVideo) return nuy.sendMessage(from, 'Reply videonya!',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
					svst = body.slice(10)
					if (!svst) return nuy.sendMessage(from, 'Nama videonya apa?',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
					boij = JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo
					delb = await nuy.downloadMediaMessage(boij)
					videonye.push(`${svst}`)
             ...
             case 'vidlist':
				...
				if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					...
					nuy.sendMessage(from, teks.trim(), extendedText, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true, contextInfo: { "mentionedJid": videonye } }})
					break
             case 'getvid':
             case 'gvid':
                    if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					...
					nuy.sendMessage(from, buffer, video, { mimetype: 'video/mp4', quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true }})
					break
					case 'playstore':
		if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
                    if (args.length == 0) return nuy.sendMessage(from, `Example: ${prefix + command} telegram`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
                    query = args.join(" ")
                    ...
                    case 'alquranaudio':
		if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
                    if (args.length == 0) return nuy.sendMessage(from, `Example: ${prefix + command} 18 or ${prefix + command} 18/10`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
                    surah = args[0]
                    ini_buffer = await getBuffer(`http://api.lolhuman.xyz/api/quran/audio/${surah}?apikey=${LoLKey}`)
                    nuy.sendMessage(from, ini_buffer, audio, { quoted: mek2, mimetype: Mimetype.mp4Audio })
                    break
                case 'asmaulhusna':
		if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
                ...
                case 'kisahnabi':
		if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
                    if (args.length == 0) return nuy.sendMessage(from, `Example: ${prefix + command} Muhammad`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
                    query = args.join(" ")
                ...
                case 'jadwalsholat':
		if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
                    if (args.length == 0) return nuy.sendMessage(from, `Example: ${prefix + command} Banten`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
                    daerah = args.join(" ")
                    ...
                    case 'stickerwa':
		             if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
                    if (args.length == 0) return nuy.sendMessage(from, `Example: ${prefix + command} Koceng Imot`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
                    query = args.join(" ")
                    ...
                    for (var x of get_result) {
                        ...
                        nuy.sendMessage(from, ini_buffer, sticker, {quoted: mek2})
                    }
                    break
					case 'wallpapersearch':
		            if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
                    if (args.length == 0) return nuy.sendMessage(from, `Example: ${prefix + command} loli`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
                    query = args.join(" ")
				...
				username = args[0]
					...
					nuy.sendMessage(from, profile, image, {quoted: mek, caption: txt})
					nuy.sendMessage(from, banner, image, {quoted: mek2, caption: 'BANNERNYA OM!' })
					break
		...
		case 'donate':
                 if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					nuy.sendMessage(from, donasi(), MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')}})
					break
		case 'bahasa':
                 if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				if (isBanned) return nuy.sendMessage(from, `Maaf ${pushname} kamu sudah terbenned!`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					nuy.sendMessage(from, bahasa(), MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
					await limitAdd(sender)
					break
		case 'kodenegara':
                 ...
                 if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				if (isBanned) return nuy.sendMessage(from, `Maaf ${pushname} kamu sudah terbenned!`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					nuy.sendMessage(from, negara(), MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
					await limitAdd(sender)
					break
		case 'del':
		case 'd':
		...
		case 'say':
                 ...
                 if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
                                        if (args.length < 1) return nuy.sendMessage(from, 'Teksnya mana kak?',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
                                        saying = teks
                                        nuy.sendMessage(from, saying, MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
					await limitAdd(sender)
                                        break
		case 'info':
					me = nuy.user
				    ...
				    nuy.sendMessage(from, daca, image, {quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true, mentionedJid: [ow]}, caption: teks})
					break 
		case 'snk':
					me = nuy.user
					uptime = process.uptime()
		...
		case 'lpr':
                ...
                if (args.length < 1) return reply(`Yang mau direport apaan?`)
                     const pesan = body.slice(5)
                      if (pesan.length > 300) return nuy.sendMessage(from, 'Maaf Teks Terlalu Panjang, Maksimal 300 Teks', text, {quoted: mek2})
                    ...
                    nuy.sendMessage('6283815956151@s.whatsapp.net', options, text, {quoted: mek})
                    nuy.sendMessage(from, 'Masalah telah di laporkan ke owner BOT, laporan palsu/main2 tidak akan ditanggapi!!',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
                    break
		case 'request':
					...
					nuy.sendMessage('6283815956151@s.whatsapp.net', options, text, {quoted: mek})
					nuy.sendMessage(from, 'REQUEST ANDA TELAH SAMPAI KE OWNER MANCA, Requests palsu atau main² tidak akan ditanggapi!!',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
					break
		case 'blocklist': 
					...
					nuy.sendMessage(from, teks.trim(), extendedText, {quoted: mek2, contextInfo: {"mentionedJid": blocked}})
					break
                case 'hidetag':
					// NuyFaa
                 if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				if (isBanned) return nuy.sendMessage(from, `Maaf ${pushname} kamu sudah terbenned!`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					...
					nuy.sendMessage(from, options, MessageType.text, {quoted: ftroli, sendEphemeral: true, contextInfo: { forwardingScore: 508, isForwarded: true}, thumbnail: fs.readFileSync('./src/pinky.jpg', 'base64')})
					await limitAdd(sender)
					break
				...
				if (isBanned) return nuy.sendMessage(from, `Maaf ${pushname} kamu sudah terbenned!`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
                                ...
                                const tagzz = {
                                        ...
                                        nuy.sendMessage(from, tagzz, MessageType.text, {quoted: mek2})
					await limitAdd(sender)
                                        break
                                        case 'profile':
					...
					nuy.sendMessage(from, buffer, image, {quoted: mek2, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64'), caption: profile})
		...
		case 'quotesmotivasi':
				...
				if (isBanned) return nuy.sendMessage(from, `Maaf ${pushname} kamu sudah terbenned!`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
	                                const kucings = ['https://i.ibb.co/vvmp82w/kucing1.jpg']
                                        ...
                                        nuy.sendMessage(from, kucigz, image, {quote: mek})
					await limitAdd(sender)
                                        break*/
		case 'ssweb':
		...
		case 'hentai2':
				...
				if (isBanned) return nuy.sendMessage(from, `Maaf ${pushname} kamu sudah terbenned!`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					...
					nuy.sendMessage(from, hasil, image, {quoted: mek2})
					await limitAdd(sender)
					break
		case 'animecry':
		...
		case 'kelinci':
				...
				//anime
                		case 'dewabatch':
                    			...
                    			nuy.sendMessage(from, thum, image, {quoted: mek, caption: `${anu.result}`})
                 			break
			   case 'animequotes':
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								...
								exec(`webpmux -set exif ${addMetadata(namo, ator)} ${ran} -o ${ran}`, async (error) => {
									nuy.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: ftroli, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
									fs.unlinkSync(media)	
									fs.unlinkSync(ran)	
								})
							})
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedSticker) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								fs.unlinkSync(media)
								tipe = media.endsWith('.mp4') ? 'video' : 'gif'
								nuy.sendMessage(from, `Gagal, pada saat mengkonversi ${tipe} ke stiker`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
							})
							.on('end', function () {
								console.log('Finish')
								exec(`webpmux -set exif ${addMetadata(namo, ator)} ${ran} -o ${ran}`, async (error) => {
						...
						await removeBackgroundFromImageFile({path: media, apiKey: keyrmbg, size: 'auto', type: 'auto', ranp}).then(res => {
							...
							fs.writeFileSync(ranp, buffer, (err) => {
								if (err) return nuy.sendMessage(from, 'Gagal, Terjadi kesalahan, silahkan coba beberapa saat lagi',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
							})
						...
						nuy.sendMessage(from, `reply sticker dengan caption ${prefix}colong`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
					}
					break
         case 'spatrick':
					if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
        ...
        case 'semoji':
                   if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				if (isBanned) return nuy.sendMessage(from, `Maaf ${pushname} kamu sudah terbenned!`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
                    if (args.length == 0) return nuy.sendMessage(from, `Contoh: ${prefix + command} 🤔`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
                    emoji = args[0]
                    try {
				...
				if (args.length < 1) return nuy.sendMessage(from, 'Diperlukan kode bahasa kak!!', text, {quoted: mek})
					...
					: gtts.save(ranm, dtt, function() {
						exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
							...
							buffer = fs.readFileSync(rano)
							if (err) return reply(ind.stikga())
							nuy.sendMessage(from, buffer, audio, {quoted: mek2, ptt:true})
							fs.unlinkSync(rano)
						})
					}) 
					await limitAdd(sender)
		...
		case 'setprefix':
					...
					if (!isOwner) return nuy.sendMessage(from, '「 OWNER BOT ONLY 」',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					prefix = args[0]
					nuy.sendMessage(from, `*Prefix berhasil di ubah menjadi* : ${prefix}`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					break 
		case 'setprefixx':
					if (args.length < 1) return
					if (!isAdmin) return nuy.sendMessage(from, 'Only Admin Bot',MessageType.text, { quoted: ftroli, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
		...
		case 'setpinky':
					...
					if (!isOwner) return nuy.sendMessage(from, '「 OWNER BOT ONLY 」',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					pinky = args.join(" ")
					nuy.sendMessage(from, `*Link Menu Bot berhasil di ubah menjadi* : ${pinky}`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					break 
		case 'setlimit':
		case 'addlimit':
					if (args.length < 1) return
					if (!isOwner) return nuy.sendMessage(from, '「 OWNER BOT ONLY 」',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					limitawal = args[0]
					nuy.sendMessage(from, `*Limit berhasil di ubah menjadi* : ${limitawal}`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					break 
		case 'setlimitt':
		case 'addlimitt':
					if (args.length < 1) return
		...
		case 'setmemlimit':
					...
					memberlimit = args[0]
					nuy.sendMessage(from, `Change Member limit To ${memberlimit} SUCCESS!`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					break 
		case 'setmemlimitt':
					if (args.length < 1) return
				    if (!isAdmin) return nuy.sendMessage(from, 'Only Admin Bot',MessageType.text, { quoted: ftroli, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
		...
		case 'tiktokstalk':
				...
				if (isBanned) return nuy.sendMessage(from, `Maaf ${pushname} kamu sudah terbenned!`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				try {
						if (args.length < 1) return nuy.sendMessage(from, 'Username mana kak?',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				...
				nuy.sendMessage(from, yeh, MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				await limitAdd(sender)
				break
		case 'hargaprem':
		case 'hargapremium':
					nuy.sendMessage(from, 'Level 100 = Free 3 day (premium)\n10k = 30day (premium)',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					break
		case 'buypremium':
					nuy.sendMessage(from, 'Ingin membeli premium?\nHarap hubungi kami : wa.me/6283815956151',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					break
		...
		case 'wame':
                 ...
                 if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
  					...
  					nuy.sendMessage(from, options, MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
  					break
  					if (data.error) return reply(data.error)
  					reply(data.result)
					await limitAdd(sender)
		...
		case 'blockk':
				...
				if (!isAdmin) return nuy.sendMessage(from, 'Only Admin Bot',MessageType.text, { quoted: ftroli, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					...
					nuy.sendMessage(from, `*Perintah Diterima, Memblokir* ${body.slice(7)}@c.us`, MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					break
		case 'unblockk':
					if (!isGroup) return reply(ind.groupo())
				if (!isAdmin) return nuy.sendMessage(from, 'Only Admin Bot',MessageType.text, { quoted: ftroli, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				    nuy.blockUser (`${body.slice(10)}@c.us`, "remove")
					nuy.sendMessage(from, `*Perintah Diterima, Membuka Blockir* ${body.slice(9)}@c.us`, MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					break
		case 'block':
				 nuy.updatePresence(from, Presence.composing) 
				 nuy.chatRead (from)
		...
		case 'outmanca':
			if (!isOwner) return nuy.sendMessage(from, '「 OWNER BOT ONLY 」',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')}) 
                      ...
                      setTimeout( () => {
                      nuy.updatePresence(from, Presence.composing) 
                      nuy.sendMessage(from, 'Sukses Owner😉',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
                      }, 0)
                      break			
		case 'bc': 
					if (!isOwner) return nuy.sendMessage(from, '「 OWNER BOT ONLY 」',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')}) 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							nuy.sendMessage(_.jid, buff, image, {quoted: {key : {participant : '0@s.whatsapp.net'}, message: {orderMessage: {itemCount : 1, status: 1, surface : 1, message: 'BC BY : OWNER', orderTitle: 'BC BY : OWNER', thumbnail: fs.readFileSync('pinky.jpg'), sellerJid: '0@s.whatsapp.net'} } }, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64'), caption: `*「 PESAN BROADCAST 」*\n\n${body.slice(4)}`})
						}
						nuy.sendMessage(from, '*Suksess broadcast*',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					} else {
						for (let _ of anu) {
		...
		case 'setpp': 
					...
					nuy.sendMessage(from, '*Sukses mengganti icon group*',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					break				
		case 'add':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (args.length < 1) return nuy.sendMessage(from, 'Yang mau di add tuyul ya?',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					if (args[0].startsWith('08')) return nuy.sendMessage(from, 'Gunakan kode negara kak',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					try {
		...
		case 'group':
					...
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (args[0] === 'buka') {
					    nuy.sendMessage(from, '*BERHASIL MEMBUKA GROUP*',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
						nuy.groupSettingChange(from, GroupSettingChange.messageSend, false)
					} else if (args[0] === 'tutup') {
						nuy.sendMessage(from, '*BERHASIL MENUTUP GROUP*',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
		...
		case 'tutuptime': 
		 ...
		 case 'infogc':
		    	...
		    	if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				if (isBanned) return nuy.sendMessage(from, `Maaf ${pushname} kamu sudah terbenned!`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
                if (!isGroup) return nuy.sendMessage(from, 'Only Group',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
                nuy.updatePresence(from, Presence.composing)
                try {
		        ...
		        nuy.sendMessage(from, buffer, image, {quoted: mek2, caption: `*NAME* : ${groupName}\n*MEMBER* : ${groupMembers.length}\n*ADMIN* : ${groupAdmins.length}\n*DESK* : ${groupDesc}`})
                break
            case 'admin':
            case 'owner':
            case 'creator':
            case 'developer':
                  nuy.sendMessage(from, {displayname: "Jeff", vcard: vcard}, MessageType.contact, { quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: {"contactMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": fakereply, "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('./pinky.jpg')}}}, contextInfo: { forwardingScore: 508, isForwarded: true}})
                  nuy.sendMessage(from, 'Tuh Kontak Owner Manca, Jangan Spam Vc dll Chat Ga Pph Asal Jangan Jorok!!',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					break    
           case 'kick':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return nuy.sendMessage(from, 'BOT JADIIN ADMIN DULU',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
		...
		case 'demote':
           	...
           	case 'setname':
                ...
                nuy.sendMessage(from, 'Succes, Ganti Nama Grup',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					break
                case 'setdesc':
                if (!isGroup) return reply(ind.groupo())
			    if (!isGroupAdmins) return reply(ind.admin())
                ...
                nuy.sendMessage(from, 'Succes, Ganti Deskripsi Grup',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					break
		case 'listadmin':
					if (!isGroup) return reply(ind.groupo())
					...
					nuy.sendMessage(from, bur, MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					break
      case 'upswimg':
					nuy.updatePresence(from, Presence.composing)
					if (isQuotedImage) {
		...
		case 'toimg':
                 ...
                 if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				if (!isQuotedSticker) return nuy.sendMessage(from, '*Reply/Tag sticker!*',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					nuy.sendMessage(from, 'Sedang Diproses...',MessageType.text, { quoted: fgif, contextInfo: { forwardingScore: 508, isForwarded: true}})
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await nuy.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.png')
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						if (err) return reply(ind.stikga())
						buffer = fs.readFileSync(ran)
						nuy.sendMessage(from, buffer, image, {quoted: mek2, caption: '*Nih Kack...*'})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender)
					break
		case 'nsfw':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					if (args.length < 1) return nuy.sendMessage(from, 'Mengaktifkan Ketik 1, Menonaktif Ketik 0',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					if (args[0] === 'on') {
						if (isNsfw) return nuy.sendMessage(from, '*Fitur nsfw sudah aktif sebelum nya*',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
						nsfw.push(from)
						fs.writeFileSync('./database/bot/nsfw.json', JSON.stringify(nsfw))
						nuy.sendMessage(from, '❬ SUCCSESS ❭ mengaktifkan mode nsfw di group ini ✔',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					} else if (args[0] === 'off') {
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/bot/nsfw.json', JSON.stringify(nsfw))
						nuy.sendMessage(from, '❬ SUCCSESS ❭ menonaktifkan mode nsfw di group ini ✔',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					} else {
						reply(ind.satukos())
					}
					break
		case 'leveling':
					if (!isGroup) return reply(ind.groupo())
					if (!isOwner) return nuy.sendMessage(from, '「 OWNER BOT ONLY 」',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					if (args.length < 1) return nuy.sendMessage(from, 'Mengaktifkan Ketik On, Menonaktif Ketik Off',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					if (args[0] === 'on') {
					if (isLevelingOn) return nuy.sendMessage(from, '*Fitur level sudah aktif sebelum nya*',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					_leveling.push(from)
		...
		case 'welcome':
					...
					if (args.length < 1) return nuy.sendMessage(from, 'Mengaktifkan Ketik On, Menonaktif Ketik Off',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					if (args[0] === 'on') {
						if (isWelkom) return nuy.sendMessage(from, '*Fitur welcome sudah aktif sebelum nya',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
						welkom.push(from)
						fs.writeFileSync('./database/bot/welkom.json', JSON.stringify(welkom))
						nuy.sendMessage(from, '❬ SUCCSESS ❭ mengaktifkan fitur welcome di group ini',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					} else if (args[0] === 'off') {
						welkom.splice(from, 1)
						fs.writeFileSync('./database/bot/welkom.json', JSON.stringify(welkom))
						nuy.sendMessage(from, '❬ SUCCSESS ❭ menonaktifkan fitur welcome di group ini',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					} else {
						reply(ind.satukos())
					}
					break
                 case 'antilink':
					...
					if (args.length < 1) return nuy.sendMessage(from, 'Mengaktifkan Ketik On, Menonaktif Ketik Off',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					if (args[0] === 'on') {
						if (isAntilink) return nuy.sendMessage(from, 'Anti link group sudah aktif',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
						antilink.push(from)
						fs.writeFileSync('./database/kelompok/antilink.json', JSON.stringify(antilink))
						nuy.sendMessage(from, '❬ SUCCSESS ❭ mengaktifkan anti link group di group ini ✔',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
						nuy.sendMessage(from, 'Perhatian kepada seluruh member anti link group aktif apabila anda mengirim link group anda akan di kick dari group',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					} else if (args[0] === 'off') {
						if (!isAntilink) return reply('Mode anti link group sudah disable')
						antilink.splice(from, 1)
						fs.writeFileSync('./database/kelompok/antilink.json', JSON.stringify(antilink))
						nuy.sendMessage(from, '❬ SUCCSESS ❭ menonaktifkan anti link group di group ini ✔',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					} else {
						nuy.sendMessage(from, 'Mengaktifkan Ketik On, Menonaktif Ketik Off',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					}
					break
		case 'clone':
					if (!isGroup) return reply(ind.groupo())
					if (!isOwner) return reply(ind.ownerg()) 
					if (args.length < 1) return nuy.sendMessage(from, '*TAG YANG MAU DI CLONE!!!*',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return nuy.sendMessage(from, '❬ SUCCSESS ❭',MessageType.text, { quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./pinky.jpg`, 'base64')})
				...
				case 'wait':
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await wait(media).then(res => {
							nuy.sendMessage(from, res.video, video, {quoted: mek2, caption: res.teks.trim()})
						}).catch(err => {
							reply(err)
						})
					} else {
